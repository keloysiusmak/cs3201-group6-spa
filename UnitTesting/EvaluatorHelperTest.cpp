...
namespace EvaluatorHelperTest {
  ...
  TEST_CLASS(EvaluatorHelperTest) {
    ...
    TEST_METHOD(EvaluatorHelperMergeWithOverlapOneParamTest) {
      ...
      /* ClauseResults Instantiation */
      ClauseResults clauseResults = createClauseResult(Follows, ASSIGN, "a2", STMT, "s3");
    ...
    TEST_METHOD(EvaluatorHelperMergeWithOverlapTwoParamTest) {
      ...
      /* ClauseResults Instantiation */
      ClauseResults clauseResults = createClauseResult(Follows, ASSIGN, "a2", STMT, "s");
      vector<vector<int>> clauseResultsTable = { {2, 4}, {3, 5} };
      clauseResults.setResults(clauseResultsTable);
	...
	{3, 4, 4, 3},
      ...
      iTable.setResults(unsortedResults);

      Param paramToSortBy = Param(ASSIGN, "a2");
	...
	{3, 9, 8, 9},
      ...
      Assert::AreEqual(true, unsortedResults, sortedResults);
