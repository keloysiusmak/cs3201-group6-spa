...
std::vector<std::vector<int>> PKB::getAffectsBeforeStar(int stmt) {
	...
	std::vector<std::vector<int>> preOutput = PKB::getAllAffectsStar();
	std::vector<std::vector<int>> output;
	for (std::vector<int> i : preOutput) {
		if (i[1] == stmt) {
...
std::vector<int> PKB::processAffectsOrder(int first) {
	std::vector<int> output;
	std::vector<int> recurOutput;
	std::vector<std::vector<int>> next;
	if (PKB::checkStatementHasType(first, 1) || PKB::checkStatementHasType(first, 4)) {
		output.push_back(first);
		next = PKB::getNextAfter(first);
		if (next.size() > 0 && next[0][0] > first) {
			recurOutput = PKB::processAffectsOrder(next[0][0]);
		}
		output.insert(output.end(), recurOutput.begin(), recurOutput.end());
	}
	else if (PKB::checkStatementHasType(first, 2)) {
		next = PKB::getNextAfter(first);
		std::vector<int> temp;
		for (std::vector<int> t : next) {
			if (t[0] > first) {
				temp.push_back(t[0]);
			}
		}
		...
		output.push_back(first);
		recurOutput = PKB::processAffectsOrder(temp[0]);
		output.insert(output.end(), recurOutput.begin(), recurOutput.end());
		output.push_back(first);
		recurOutput = PKB::processAffectsOrder(temp[0]);
		output.insert(output.end(), recurOutput.begin(), recurOutput.end());
		if (temp.size() > 1) {
			output.push_back(first);
			if (temp[1] > first) {
				recurOutput = PKB::processAffectsOrder(temp[1]);
			}
			output.insert(output.end(), recurOutput.begin(), recurOutput.end());
		}
	}
	else if (PKB::checkStatementHasType(first, 3)) {
		next = PKB::getNextAfter(first);
		std::vector<int> temp;
		for (std::vector<int> t : next) {
			temp.push_back(t[0]);
...
std::vector<std::vector<int>> PKB::getAllAffects() {
	...
	for (int i = 0; i < static_cast<int>(procs.size()); i++) {
		...
		std::vector<int> next = PKB::processAffectsOrder(firstStmt);
		while (next.size() > 0) {
			currStmt = next.front();
...
std::vector<std::vector<int>> PKB::getAllAffectsStar() {
	...
	while (initialOutputSize != finalOutputSize) {
		unordered_map<int, vector<int>> outputMap;
		initialOutputSize = output.size();
		for (std::vector<int> i : output) {
			unordered_map<int, vector<int>>::iterator got = outputMap.find(i[0]);

			if (got == outputMap.end()) {
				outputMap.insert({ i[0],{ i[1] } });
			}
			else {
				outputMap[i[0]].push_back(i[1]);
			}
		}
		std::vector<std::vector<int>> finalOutput;
		for (auto it = outputMap.begin(); it != outputMap.end(); ++it) {
			for (int i : it->second) {
				finalOutput.push_back({it->first, i});
				unordered_map<int, vector<int>>::iterator got2 = outputMap.find(i);
				if (got2 != outputMap.end()) {
					for (int j : got2->second) {
						finalOutput.push_back({ it->first, j });
					}
				}
			}
		}
