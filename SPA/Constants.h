#pragma once

const int STATEMENT_TABLE = 1;
const int STATEMENT_LIST_TABLE = 2;
const int PROC_INFO_TABLE = 3;
const int USES_TABLE = 4;
const int MODIFIES_TABLE = 5;
const int PATTERN_TABLE = 6;
const int CONST_TABLE = 7;
const int CALLS_TABLE = 8;
const int CALLS_STAR_TABLE = 9;
const int CALLS_INVERSE_TABLE = 10;
const int CALLS_STAR_INVERSE_TABLE = 11;
const int NEXT_TABLE = 12;
const int NEXT_STAR_TABLE = 13;
const int NEXT_INVERSE_TABLE = 14;
const int NEXT_STAR_INVERSE_TABLE = 15;
const int PROC_TABLE = 16;
const int VAR_TABLE = 17;